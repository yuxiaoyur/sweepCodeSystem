    ...
    methods:{
        ...
        getZzjgCheckData(data,check){
            ...
            if(checkData.ischeck===true){//选中
                ...
                // checkData.data.map((item,index)=>{
                //     this.zzjgCheckData.map((im,idx)=>{
                //         // console.log(item.code,im.code)
                //         if(im.code===item.code){
                //             this.zzjgCheckData.splice(idx,1)
                //         }
                //     })
                // })
                // if(this.zzjgCheckData.length>0){
                //     checkData.data.map((item,index)=>{
                //         this.zzjgCheckData.map((im,idx)=>{
                //             if(im.code===item.code){
                //                 this.zzjgCheckData.splice(idx,1)
        ...
        getXzqyCheckData(data,check){
            ...
            if(checkData.ischeck===true){//选中
                checkData.data.map((item,index)=>{
                    // this.xzqyCheckData.push(item)
                    this.sjqxruleForm.xzqyqx.push(item.code)
                })
            }else{//取消选中
                // if(this.xzqyCheckData.length>0){
                //     checkData.data.map((item,index)=>{
                //         this.xzqyCheckData.map((im,idx)=>{
                //             if(im.code===item.code){
                //                 this.xzqyCheckData.splice(idx,1)
                //             }
                //         })
                //     })
                // }
