  ...
  <el-dialog
    ...
    <div class="mod-mssq-rcfx">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        <el-form-item>
          <el-input v-model="dataForm.zsnr" placeholder="内容关键词" clearable></el-input>
        </el-form-item>
        <el-form-item>
          <el-input v-model="dataForm.zssj" placeholder="指示时间(yyy-mm-dd)" clearable></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button v-if="hasPermission('update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id,scope.row.zsnr,'update')">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
<script>
  import AddOrUpdateLdzs from './ldzs-addOrUpdate.vue'
  export default {
    data () {
      return {
        visible: false,
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/yjzh/findldzs'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.pageIndex,
            'limit': this.pageSize,
            'zsnr': this.dataForm.zsnr,
            'zssj': this.dataForm.zssj,
      ...
      addOrUpdateHandle (id, zsnr, subType) {
        this.addOrUpdateVisible = true
        this.$nextTick(() => {
          this.$refs.addOrUpdate.init(id, zsnr, subType)
        })
      },
      // 删除
      deleteHandle (id) {
        ...
        this.$confirm(`确定进行[${rcIds ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/yjzh/removeLdzs'),
            method: 'post',
