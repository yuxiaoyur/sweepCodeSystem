<template>
  <el-collapse v-model="activeNames">
    <el-collapse-item title="已绑定库表" name="1">
      <el-form :inline="true" :model="dbInfo" @keyup.enter.native="getDataList()">
        <el-row>
          ...
          <el-col :span="3">
            <el-tag type="success">{{dbInfo.exhibitionDbDept}}</el-tag>
          </el-col>
          ...
          <el-button
            ...
            icon="el-icon-delete"
            circle
            @click="unbindDbInfo(dbInfo.srcId,dataAccessWay)">
          </el-button>
        </el-row>
      </el-form>
    </el-collapse-item>
    ...
    <el-collapse-item title="筛选条件" name="2">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        <el-row>
          ...
          <el-col :span="4">
            <el-form-item>
              <el-select v-model="dataForm.exhibitionDbDeptId" filterable placeholder="请选择数据来源部门" clearable>
                <el-option
    ...
    <!--      第三部分，库表记录-->
    <el-collapse-item title="库表记录" name="3">
      <el-table
        :data="dataList"
        border
  ...
  export default {
    ...
    methods: {
      setBindDeleteEvent(srcId, dataAccessWay) {
        console.log("child db - 2 - " + srcId + " " + dataAccessWay)
      ...
      bindDbInfo(moduleId, srcId, dataAccessWay, srcBody) {
        ...
        if (this.dbInfo.dbName !== '暂无') {
          ...
          this.$confirm(`确定进行[绑定]操作?`, '提示', {
            ...
            console.log("++++++++srcBody: " + srcBody + "scrId: " + srcId)
            this.dbInfo = srcBody
            this.$emit("bindUpdateEvent", this.dbInfo.id, false)
          })
        }
      },
      // 解除绑定所用函数
      unbindDbInfo(srcId, dataAccessWay) {
        ...
        if (this.dbInfo.dbName === '暂无' || this.dbInfo === undefined) {
          ...
          this.$confirm(`确定进行[解绑]操作?`, '提示', {
            ...
            console.log("db - child : unbindDbInfo1   " + srcId + "   " + dataAccessWay)
        ...
        this.$http({
          ...
          if (data && data.successful) {
            if (data.data === null) {
              ...
              console.log("从数据库获取绑定数据" + this.dbInfo)
            }
          } else {
